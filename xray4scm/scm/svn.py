...
class SVNLogClient:
    ...
    def getRevFileDiff(self, path, revno, isabsolute=False):
        rev1 = pysvn.Revision(pysvn.opt_revision_kind.number, revno-1)
        rev2 = pysvn.Revision(pysvn.opt_revision_kind.number, revno)
        url = self.getUrl(path, isabsolute)
        diff_log = None
    ...
    def getInfo(self, path, revno, isabsolute=False):
    ...
    def isChildPath(self, filepath, isabsolute=False):
        '''
        ...
        fullpath = self.getUrl(filepath, isabsolute)

        return(fullpath.startswith(self.svnrepourl+self.svnbranch))

    def isBinaryFile(self, filepath, revno, isabsolute=False):
        '''
        ...
        '''
        rev = pysvn.Revision(pysvn.opt_revision_kind.number, revno)
        url = self.getUrl(filepath, isabsolute)
        binary = None
    ...
    def isDirectory(self, revno, changepath, changetype, isabsolute=False):
        ...
        try:
            entry = self.getInfo(changepath, revno, isabsolute)
    ...
    def _getLineCount(self, filepath, revno, isabsolute):
        linecount = 0
    ...
    def cat(self, filepath, revno, isabsolute=False):
        logging.info("Trying to get contents of %s" % (filepath))
        rev = pysvn.Revision(pysvn.opt_revision_kind.number, revno)
        url = self.getUrl(filepath, isabsolute)
        return self.svnclient.cat(url, revision = rev)

    def getLineCount(self, filepath, revno, isabsolute=False):
        linecount = 0
        if self.isBinaryFile(filepath, revno, isabsolute) == False:
            linecount = self._getLineCount(filepath, revno, isabsolute)
        return linecount
    ...
    def setBranch(self, branch):
       assert( branch != None )
       self.svnbranch = '/' + branch

    def getUrl(self, path, isabsolute=False):
        url = self.svnrepourl
        if path.strip() != "":
            if not isabsolute:
                url = self.getRootUrl() + self.svnbranch + urllib.pathname2url(path)
            else:
                url = self.getRootUrl() + urllib.pathname2url(path)
        return url

    def isRepoUrlSameAsRoot(self):
        repourl = (self.svnrepourl+self.svnbranch).rstrip('/')
        rooturl = self.getRootUrl()
...
class SVNRevLog:
    ...
    def isValidChange(self, change):
        ...
        if the repository path is same is repository 'root'
        '''
        return self.logclient.isChildPath(change['path'], True)
    ...
    def isDirectory(self, change):
        ...
        if( 'isdir' not in change):
            revno = self.getRevNo()
            isDir = self.logclient.isDirectory(revno, path, action, True)
        else:
            isDir = change['isdir']
    ...
    def getDiffLineCountForPath(self, diffCountDict, change):
        ...
        filename = filename[len(self.logclient.svnbranch):]
        if filename == "":
            filename = '/'
    ...
    def __getDiffLineCountForPath(self, change):
        ...
        if( self.isDirectory(change) == False):
            ...
            if( changetype == 'A'):
                added = self.logclient.getLineCount(filepath, revno, True)
            elif( changetype == 'D'):
                deleted = self.logclient.getLineCount(filepath, revno-1, True)
            else:
                #change type is 'changetype != 'A' and changetype != 'D'
                #directory is modified
                diff_log = self.logclient.getRevFileDiff(filepath, revno, True)
